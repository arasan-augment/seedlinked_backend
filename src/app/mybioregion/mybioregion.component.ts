...
export class MybioregionComponent implements OnInit {
    ...
    /********File Validate*******/
     allowedExtensions =  ['image/png', 'image/gif', 'image/jpeg', 'image/jpg', 'image/JPG', 'image/JPEG', 'image/GIF', 'image/PNG'];
        ...
        this.mybioregionService.getAllBioRegion()
            .subscribe(
                data => {
                    ...
                    let dynamicLat = this.latitude;
                    let dynamicLong = this.longitude;
                    let sumlat = 0;
                    let sumlng = 0;
                    let i = 1;
                    for (const region of this.allBioRegion) {
                        const markerLabel = this.getNameFromNumber(i);
                        this.markers.push({
